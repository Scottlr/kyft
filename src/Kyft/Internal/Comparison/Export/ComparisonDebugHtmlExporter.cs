...
.hero {
  padding: 28px;
  border-top: 4px solid var(--overlap);
}
...
.badge.valid { border-color: #86efac; background: #ecfdf3; color: #14532d; }
.badge.invalid { border-color: #fecaca; background: #fef2f2; color: #7f1d1d; }
.badge.live { border-color: #fed7aa; background: #fff7ed; color: #7c2d12; }
...
.track {
  ...
  border-radius: 8px;
  background:
    linear-gradient(90deg, rgba(23, 32, 27, 0.05) 1px, transparent 1px) 0 0 / 12.5% 100%,
    #f9faf9;
}
...
.bar.target { background: var(--target); }
.bar.against { background: var(--against); }
.bar.overlap { background: var(--overlap); }
.bar.residual { background: var(--residual); }
.bar.missing { background: var(--missing); }
...
.severity-error { color: var(--error); font-weight: 720; }
.severity-warning { color: var(--warning); font-weight: 720; }
...
""")
    ...
    private static void AppendHero(StringBuilder builder, ComparisonResult result)
    {
        builder
            ...
            .AppendLine("  <div class=\"eyebrow\">Kyft comparison debug</div>")
            .Append("  <h1>");
        AppendText(builder, result.Plan.Name);
        builder
            .AppendLine("</h1>")
    ...
    private static void AppendLegend(StringBuilder builder, bool includeSegments)
    {
        builder
            ...
            .AppendLine("    <span class=\"legend-item\"><span class=\"swatch\" style=\"background:var(--target)\"></span>Target window</span>")
            .AppendLine("    <span class=\"legend-item\"><span class=\"swatch\" style=\"background:var(--against)\"></span>Comparison window</span>");

        if (includeSegments)
        {
            builder
                .AppendLine("    <span class=\"legend-item\"><span class=\"swatch\" style=\"background:var(--overlap)\"></span>Overlap</span>")
                .AppendLine("    <span class=\"legend-item\"><span class=\"swatch\" style=\"background:var(--residual)\"></span>Target only</span>")
                .AppendLine("    <span class=\"legend-item\"><span class=\"swatch\" style=\"background:var(--missing)\"></span>Comparison only</span>")
                .AppendLine("    <span class=\"legend-item\"><span class=\"swatch\" style=\"background:var(--gap)\"></span>Gap</span>");
        }
